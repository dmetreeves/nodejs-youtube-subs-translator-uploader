...
export class YandexTranslator2 {
        ...
        await puppeteerNewTempPage(async (page) => {
            ...
            for(let langCode of langCodes) {
                if(langCode === 'en') {
                    ...
                    await page.goto(`https://translate.yandex.com/?lang=en-${ langCode }&text=${ encodeURIComponent( shortSrcText ) }`);
                    try {
                        ...
                        await page.screenshot({path: `./png/e.png`});
                        break;
                    }
                    
                    let outputArea = await page.$('#translation');
                    outputText = await page.evaluate(el => el.textContent, outputArea);
                }
