...
function getLangsEndpoint(translator) {
    return (req, res) => {
        const enLangsPerCodes = translator.yandexAllowedLangNamesEn();
        const ruLangsPerCodes = translator.yandexAllowedLangNamesRu();
        const rows = Object.entries(enLangsPerCodes).map(
            ([enCode, enLang]) =>
                `<tr><td>${ ruLangsPerCodes[enCode] }</td><td>${ enLang }</td></tr>`
        ).join('');
